-- 20260717181303_claim_account_and_incomplete_users
...
create or replace function public.admin_list_incomplete_users()
returns json language plpgsql security definer set search_path = public, extensions
as $$
declare result json;
begin
  select coalesce(json_agg(row_to_json(x)), '[]'::json) into result from (
    select id, card_id, display_name, nome, cognome, balance,
           codice_fiscale, gdpr_privacy_letta, gdpr_trattamento,
           created_at, registered_at
    from public.users
    where role = 'user'
      and (codice_fiscale is null or codice_fiscale = ''
           or gdpr_privacy_letta = false)
    order by card_id
  ) x;
  return result;
end $$;
